...
def _force_test_numbers(monkeypatch):
    ...
    monkeypatch.setattr(get_settings(), "otp_test_numbers", "+998900000000:00000")


async def test_issue_returns_fixed_code_for_test_number():
    assert await otp.issue("+998900000000") == "00000"


async def test_verify_accepts_fixed_code():
    assert await otp.verify("+998900000000", "00000") is True
